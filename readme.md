...
2. T-Shirt Sizes Items are estimated into t-shirt sizes: XS, S, M, L, XL. The decision about the size is based on an open and mutual collaborative discussion. This method is an informal and quick way to get an rough feeling about the total size of your backlog. More about T-shirt size estimation is here.
3. Dot Voting Each person gets a small number of small stickers and can choose to vote for the individual items. The more dots is an indicator of a bigger size. Works well in both small and large group. You have to limit the number of estimated items.
4. The Bucket System Беруться бакети від 1 до 8, спочатку виберається одна таска, естімейтається і її ложать в бакет, потім всі остальні таскі рандомно розділяють між всіма учасниками і кожен учасник ложить таску в той бакет якій вважає потрібним, потім всі ревьювають таскі.
5. Large/Uncertain/Small The team is being asked to place the items in one of these categories. The first step is to categorize the obvious items in the two extreme categories. Next the group can discuss the more complex items. This is actually a simplification of the bucket system. The system is especially good to use in smaller groups with comparable items. Next you can assign sizes to these 3 categories.
